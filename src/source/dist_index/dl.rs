...
use crate::source::DistIndexError;
...
pub(in crate::source::dist_index) fn fetch() -> TResult<Document> {
    ...
    let mut http_client = HttpClient::new().map_err(DistIndexError::from)?;
    ...
    Ok(Document::LocalPath(PathBuf::from(output_path)))
}
...
fn write_objects(
    ...
    for object in objects {
        ...
        let size = object.size.unwrap_or(-1);

        let _ = buffer.write(format!("{};{};{}\n", key, modified, size).as_bytes());
    }
